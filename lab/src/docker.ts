import { exec, execStream, waitFor, log, logError, retry } from './common.js';
  ...
  try {
    ...
    try {
      ...
      if (composePath.includes('/mnt/c/')) {
        throw new Error(
          'docker-compose resolves to Windows binary. Install docker compose plugin instead.'
        );
      }
      return 'docker-compose';
    } catch {
...
export async function up(rootDir: string): Promise<void> {
  await checkDocker();
  ...
  try {
    await execStream(`${composeCmd} -f "${composeFile}" up -d`, rootDir, (line) =>
      log('docker', line)
    );
  } catch (err: any) {
  ...
  await retry(
    async () => {
      ...
      if (!ready) throw new Error('Synapse not ready');
    },
...
export async function down(rootDir: string): Promise<void> {
  const composeCmd = await getComposeCommand();
  const composeFile = join(rootDir, 'infra', 'docker-compose.yml');

  log('docker', 'stopping synapse + element');

  await execStream(`${composeCmd} -f "${composeFile}" down`, rootDir, (line) =>
    log('docker', line)
  );
