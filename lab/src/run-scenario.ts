...
import { readEnvFile } from './common.js';
...
import { spawnGateway, stopGateway, stopGatewayByProfile, GatewayProcess } from './gateway.js';
...
async function main() {
  ...
  // Create DM room
  const dmMeta = await createDmRoom(
    bootstrapResult.homeserver,
    bootstrapResult.sellerMxid,
    bootstrapResult.sellerToken,
    bootstrapResult.buyerMxid,
    bootstrapResult.buyerToken,
    runId,
    outDir
  );
  ...
  await injectMission(
    ...
    `NUDGE: Go to #market:localhost now, find the latest listing with RUN_ID:${runId}, and DM the seller immediately.`,
    buyerGatewayUrl,
    buyerToken
  );
