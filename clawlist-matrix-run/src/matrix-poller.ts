...
import { createClient, RoomEvent } from './matrix-api.js';
...
interface BuyerConfig {
  profile: string;
  interests: string[]; // Keywords to match (e.g., ["switch", "nintendo"])
  mxid: string;
  accessToken: string;
  gatewayUrl: string; // e.g., http://127.0.0.1:18810
  gatewayToken: string;
}

interface PollerState {
  nextBatch?: string;
  seenMessages: Set<string>; // event_id set
  lastPollTime: number;
}

export class MatrixPoller {
  private homeserver: string;
  private marketRoomId: string;
  private buyers: BuyerConfig[];
  ...
  constructor(
    ...
    buyers: BuyerConfig[],
    stateFile: string,
    pollInterval: number = 30000 // 30s default
  ) {
  ...
  async loadState(): Promise<void> {
    try {
      ...
      if (data.nextBatch) this.state.nextBatch = data.nextBatch;
  ...
  async triggerAgent(buyer: BuyerConfig, listing: RoomEvent): Promise<void> {
...
Action: DM the seller now to start negotiating. Ask about condition, accessories, price, and pickup location.`;
      ...
      await exec(
        `openclaw --profile "${buyer.profile}" system event ` +
  ...
  private async processEvent(event: RoomEvent): Promise<void> {
